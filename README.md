...
- La carpeta [detacciones_crudas](https://github.com/sanruizguz/Escuchaton/tree/main/Detecciones_crudas) deben contener las detecciones sin estandarizar
- La carpeta [detacciones_estandarizadas](https://github.com/sanruizguz/Escuchaton/tree/main/Detecciones_estandarizadas) deben conectar las detecciones corregidas bajo el mismo formato

- La carpeta [detacciones_filtradas](https://github.com/sanruizguz/Escuchaton/tree/main/Detecciones_filtradas) va a contener un archivo csv por cada especie incluida en la lista de [especies objetivo](https://github.com/sanruizguz/Escuchaton/blob/main/especies_objetivo.csv) con el mismo número de grabaciones, por ejemplo (basado en el puntaje de detección arrojado por BirdNET). Estos últimos son los que deben ser descargados para iniciar el proceso de verificación. 
...
| **Columna**         | **Descripción** |
|---------------------|-----------------|
| **filePath**        | Ruta del archivo de audio original |
| **mediaID**         | Nombre del audio. El formato usualmente se compone de 3 partes divididas por "_" (ej. `A6B8FF18_20250131_105500.WAV`):<br> - Identificador único de la grabadora usada (usualmente las letras están asociadas con el tipo de grabadora y los números con el ID).<br> - Fecha en formato Año-mes-día.<br> - Hora del día en formato Hora-minuto-segundo. |
...
| **confidence**      | Puntaje de detección generado por BirdNET, que puede interpretarse como la "confianza" del modelo en que una predicción sea correcta |
...
| **recorderModel**   | Modelo de la grabadora, incluyendo marca y versión (ej. *SMM2* para Song Meter Micro 2) |
| **locality**        | Intermedio entre punto y municipio; puede interpretarse como nombre del predio o vereda |
| **projectName**     | Nombre del proyecto; a veces contiene información geográfica o temporal (ej.: *GEOPARK_PUTUMAYO_2024*) |
...
| **segmentID**       | Identificador de la predicción de BirdNET; se compone de mediaID + eventStart + eventEnd |
